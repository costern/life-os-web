...
// Merkt sich die bisherigen Werte in prev_*, damit das Frontend Pfeile/Deltas anzeigen kann.
router.put('/', async (req, res) => {
  const b = req.body || {};
  const { rows } = await pool.query(
    `UPDATE macro_status SET
       prev_ffr = ffr, prev_ty = ty, prev_inflation = inflation, prev_real_rate = real_rate,
       ffr=$1, ffr_date=$2, ty=$3, ty_date=$4, inflation=$5, cpi_date=$6,
