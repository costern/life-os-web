...
const OVERRIDES = {
  ...
  XRP: 'ripple', ADA: 'cardano', HBAR: 'hedera-hashgraph',
  BGB: 'bitget-token', TAO: 'bittensor', HYPE: 'hyperliquid',
  XCN: 'chain-2', // Onyxcoin, ex-Chain - CoinGecko behaelt die alte ID trotz Rebrand
  GRAM: 'the-open-network', // Gram (prev. Toncoin) - CoinGecko-ID blieb beim Rebrand die alte TON-ID
...
// Coin-Icons: echte Logos von CoinGecko statt eines kuratierten Sets/veralteter CDN,
// damit auch kleinere/neuere Coins (z.B. GRAM, Kaspa, BGB, ONDO, TAO) das richtige
// Icon zeigen - ueber dieselbe Ticker->ID-Aufloesung wie bei den Preisen, damit
// Icon und Kurs garantiert vom selben Coin stammen. Sehr lange gecacht (Logos
// aendern sich praktisch nie), pro Ticker statt als ein gemeinsamer Cache-Eintrag.
const iconCache = new Map(); // TICKER (upper) -> { url, fetchedAt }
const ICON_TTL_MS = 7 * 24 * 60 * 60 * 1000;

async function getIcons(tickers) {
  const uniq = [...new Set(tickers.map(t => String(t).toUpperCase()).filter(Boolean))];
  const fehlend = uniq.filter(t => {
    const c = iconCache.get(t);
    return !c || (Date.now() - c.fetchedAt) > ICON_TTL_MS;
  });
  if (fehlend.length) {
    const idToTicker = await resolveIds(fehlend);
    const ids = [...idToTicker.keys()];
    for (let i = 0; i < ids.length; i += 250) { // CoinGecko-Limit pro coins/markets-Aufruf
      const batch = ids.slice(i, i + 250);
      const url = BASE + '/coins/markets?vs_currency=usd&ids=' + encodeURIComponent(batch.join(',')) +
        '&sparkline=false' + apiKeyQuery().replace('?', '&');
      const res = await fetch(url);
      if (!res.ok) continue;
      const json = await res.json();
      for (const c of json) {
        const ticker = idToTicker.get(c.id);
        if (ticker && c.image) iconCache.set(ticker, { url: c.image, fetchedAt: Date.now() });
      }
    }
  }
  const out = {};
  for (const t of uniq) { const c = iconCache.get(t); if (c) out[t] = c.url; }
  return out;
}

// Historische Tages-Kurse fuer die Performance-Kurve (einmalige Rueckwirkend-Schaetzung).
// CoinGecko hat dafuer keinen Sammel-Endpunkt wie bei simple/price - ein Request pro
// Coin, deshalb nur fuer tatsaechlich gehaltene Ticker aufrufen und zwischen den
// Requests kurz warten, um nicht ins Rate-Limit zu laufen (das hier laeuft selten,
// nicht bei jedem Seitenaufruf).
// Rueckgabe: { TICKER: Map(dateStr -> price) | null }
async function getHistoricalDaily(tickers, days) {
  const uniq = [...new Set(tickers.map(t => String(t).toUpperCase()))];
  const idToTicker = await resolveIds(uniq);
  const out = {};
  for (const [id, ticker] of idToTicker.entries()) {
    try {
      const url = BASE + '/coins/' + id + '/market_chart?vs_currency=usd&days=' + days + '&interval=daily' + apiKeyQuery().replace('?','&');
      const res = await fetch(url);
      if (!res.ok) { out[ticker] = null; continue; }
      const json = await res.json();
      const map = new Map();
      for (const [ts, price] of (json.prices || [])) {
        map.set(new Date(ts).toISOString().slice(0, 10), price); // letzter Preis des Tages gewinnt
      }
      out[ticker] = map;
    } catch (e) { out[ticker] = null; }
    await new Promise(r => setTimeout(r, 1500));
  }
  return out;
}

module.exports = { getPrices, getHistoricalDaily, getIcons };
