...
// Klappert 90-Tage-Fenster rueckwirkend ab (Bitget erlaubt max. 90 Tage pro Request),
// bis windowsBack erreicht oder ein Fenster ganz ohne Daten kommt UND wir schon
// mindestens minEmptyStreak leere Fenster in Folge hatten (Konto-Anfang vermutet).
async function fetchAllConvertHistory(windowsBack = 12) {
  const all = [];
  const windows = [];
  const ninetyDaysMs = 90 * 24 * 60 * 60 * 1000 - 24 * 60 * 60 * 1000; // 1 Tag Puffer
  let end = Date.now();
  for (let i = 0; i < windowsBack; i++) {
    const start = end - ninetyDaysMs;
    try {
      const json = await call('GET', '/api/v2/convert/convert-record', {
        limit: '100', startTime: String(start), endTime: String(end)
      });
      const list = (json.data && json.data.dataList) || [];
      windows.push({ from: new Date(start).toISOString(), to: new Date(end).toISOString(), count: list.length });
      all.push(...list);
    } catch (e) {
      windows.push({ from: new Date(start).toISOString(), to: new Date(end).toISOString(), error: e.message, detail: e.detail || null });
    }
    end = start;
  }
  return { records: all, windows };
}

module.exports = { getBalances, getAllFills, fillToTransaction, splitSymbol, debugConvertRecord, fetchAllConvertHistory };
